...
time.sleep(1)
# driver.find_element_by_css_selector('input[placeholder="请输入用户名"]').send_keys(user)
...
time.sleep(1)
driver.find_element(By.XPATH, "//span[text()='手动入库']").click()
time.sleep(2)
...
for j in df.index:
    if "（" in str(df["名称"].loc[j]):
        ...
        driver.find_element(By.CSS_SELECTOR, 'input[placeholder="输入食材名称或分类搜索"]').clear()
        driver.find_element(By.CSS_SELECTOR, 'input[placeholder="输入食材名称或分类搜索"]').send_keys(name)
    ...
    for sec in range(pageNum):
        ...
        for i in range(1, 16):
            ...
            # 规格
            specSelector = f"form div.el-table__body-wrapper  tbody tr.el-table__row:nth-child({i}) td:nth-child(3)"
            # 单位
            unitSelector = f"form div.el-table__body-wrapper  tbody tr.el-table__row:nth-child({i}) td:nth-child(4)"
            # 添加
            addSelector = f"form div.el-table__body-wrapper  tbody tr.el-table__row:nth-child({i}) td:nth-child(5)"
            ...
            isClick = False
            # 是否需要特殊处理
            checkNum = len(result["in"][name].keys())
            for k in range(checkNum):
                temp = list(result["in"][name].keys())[k]
                if temp == "分类":
                    check1 = result["in"][name][temp]
                    if check1 == check1Text:
                        isClick = True
                    else:
                        isClick = False
                if temp == "名称":
                    check2 = result["in"][name][temp]
                    if check2 == check2Text:
                        isClick = True
                    else:
                        isClick = False
                if temp == "规格":
                    check3 = result["in"][name][temp]
                    if check3 == check3Text:
                        isClick = True
                    else:
                        isClick = False
                if temp == "单位":
                    check4 = result["in"][name][temp]
            ...
            if isClick == True:
                ...
                driver.find_element(By.XPATH, p1).send_keys(str(df["订单金额"].loc[j]))
                # 数量
                driver.find_element(By.XPATH, p2).clear()
                time.sleep(1)
                if name == "鸡蛋":
                    driver.find_element(By.XPATH, p2).send_keys(float(df["订单量"].loc[j]) * 9)
                else:
                    driver.find_element(By.XPATH, p2).send_keys(float(df["订单量"].loc[j]))
                driver.find_element(By.XPATH, p2).send_keys(Keys.TAB)
                time.sleep(1)
                flag = True
                print("已经成功添加：        ", name)
                break
